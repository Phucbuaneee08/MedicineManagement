...
import Prj2.java.model.DungCu;
import Prj2.java.model.Product;
import Prj2.java.model.Thuoc;
import Prj2.java.model.TuThuoc;
...
public class TuThuocController implements Initializable {
    ...
    @FXML private TableColumn<Product, LocalDate> expiredDate;
    ...
    private AddMedController medController;
    private AddDCController DCController;
    public TuThuoc main = new TuThuoc();

    public TuThuocController(Controller controller) throws IOException {
        FXMLLoader pane = new FXMLLoader(getClass().getResource("/Prj2/resource/view/TuThuoc.fxml"));
    ...
    public void getAddView() {
        ...
        if(choice.compareTo("Thuốc")==0){
            medController = new AddMedController(this);
            medController.showStage();
        }
        if(choice.compareTo("Dụng Cụ")==0){
            DCController = new AddDCController(this);
            DCController.showStage();
        }
    }
    public void showTuThuoc(){
        ...
        expiredDate.setCellValueFactory(cellData ->{
                    if(cellData.getValue() instanceof Thuoc){
                        return new SimpleObjectProperty<>(((Thuoc)cellData.getValue()).getExpiredDate());
                    } else {
                        return null;
                    }
                }
                    ...
                    private final MenuButton btn = new MenuButton("");
                    {
                        ...
                        m1.setOnAction(event -> {
                            ...
                            if (product instanceof Thuoc) {
                                medController = new AddMedController(TuThuocController.this, product);
                                medController.setTextField(product);
                                medController.showStage();
                            } else if (product instanceof DungCu) {
                                DCController = new AddDCController(TuThuocController.this, product);
                                DCController.setTextField(product);
                                DCController.showStage();
                            }
                        });
                        ...
                        m3.setOnAction(event -> {
                            Product product = getTableRow().getItem();
                            showDetail = new ShowDetail(product);
                            try {
